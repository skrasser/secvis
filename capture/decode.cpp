...
// $Id: decode.cpp,v 1.11 2005/02/23 23:28:59 sven Exp $
// Modifications by Sven Krasser for secvis project. Thanks for your code, Robby.
...
//printf("%ld %ld %d\n", p->pkth->ts.tv_sec, p->pkth->ts.tv_usec);

    char *pkt_buf = 0;
    unsigned int pkt_buf_len;
    if(len < MAX_PKT_BUF_LEN) {
        pkt_buf_len = len;
    } else {
        pkt_buf_len = MAX_PKT_BUF_LEN;
    }
    pkt_buf = new char[pkt_buf_len];
    memcpy(pkt_buf, pkt, pkt_buf_len);

    log_packet_info(p->iph->ip_src.s_addr, p->iph->ip_dst.s_addr, PKT_TCP, p->sp, p->dp, p->pkth->len, p->pkth->ts, pkt_buf, pkt_buf_len);
}
...
//    NETIUDP(p);
    ...
    log_packet_info(p->iph->ip_src.s_addr, p->iph->ip_dst.s_addr, PKT_UDP, p->sp, p->dp, p->pkth->len, p->pkth->ts, pkt_buf, pkt_buf_len);
    return;
}
...
/*
 * Function: DecodeIGMP(u_int8_t *, const u_int32_t, Packet *)
 *
 * Purpose: Decode the IGMP transport layer
 *
 * Arguments: pkt => ptr to the packet data
 *            len => length from here to the end of the packet
 *            p   => pointer to the decoded packet struct
 ...
 * Returns: void function
 */
void DecodeIGMP(u_int8_t * pkt, const u_int32_t len, Packet * p)
{
    u_int16_t csum;
    //u_int16_t orig_p_caplen;

    if(len < IGMP_HEADER_LEN)
    {
        //ErrorMessage("WARNING: Truncated IGMP header "
        //             "(%d bytes)\n", len);
    ...
    if(pv.checksums_mode & DO_IGMP_CHECKSUMS)
    {
        csum = in_chksum_igmp((u_int16_t *)p->igmph, len);

        if(csum)
        {
            p->csum_flags |= CSE_IGMP;

            DEBUG_WRAP(DebugMessage(DEBUG_DECODE, "Bad IGMP Checksum\n"););
        }
        else
        {
            DEBUG_WRAP(DebugMessage(DEBUG_DECODE,"IGMP Checksum: OK\n"););
        }
    }

    p->dsize = (u_short)(len - IGMP_HEADER_LEN);
    p->data = pkt + IGMP_HEADER_LEN;

    DEBUG_WRAP(DebugMessage(DEBUG_DECODE, "IGMP type: %d   mrt: %d   gaddr:%lu\n", 
                p->igmph->type, p->igmph->mrt, p->igmph->gaddr););
